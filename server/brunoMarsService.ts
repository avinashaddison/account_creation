...
export async function brunoMarsSignupFlow(
  ...
  try {
    ...
    log("🌐 Connecting to browser for Bruno Mars presale...");
    ...
    log("🔗 Navigating to Bruno Mars presale signup page...");
    ...
    for (let w = 0; w < 15; w++) {
      ...
      if (text.includes("select your events") || text.includes("sign up") || text.includes("first name")) {
        log("📄 Presale signup form loaded!");
        break;
      }
      if (text.includes("checking") || text.includes("one moment")) {
        log(`🛡️ Waiting for challenge to resolve... (${w * 2}s)`);
        continue;
      }
      if (w === 14) {
    ...
    onStatusUpdate("presale_filling");
    log("✏️ Filling in presale registration details...");
    ...
    log(firstNameFilled ? `✅ First name: ${firstName}` : "⚠️ Could not fill first name");
    ...
    log(lastNameFilled ? `✅ Last name: ${lastName}` : "⚠️ Could not fill last name");
    ...
    log(emailFilled ? `✅ Email: ${email}` : "⚠️ Could not fill email");
    ...
    if (filledCount === 0) {
      log("⚠️ No required fields filled. Trying positional fill...");
      ...
      log("Positional fill: " + JSON.stringify(positionalResult));
    }
    ...
    if (submitResult.startsWith("clicked")) {
      ...
      if (afterLower.includes("your selections") || afterLower.includes("edit selections")) {
        ...
        log("✅ SUCCESS! Presale signup confirmed - YOUR SELECTIONS page visible!");
        ...
        onStatusUpdate("completed");
        log("📄 Form submitted. Response: " + afterText.substring(0, 200).replace(/\n/g, ' '));
      }
