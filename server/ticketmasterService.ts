...
export async function tmFullRegistrationFlow(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    if (attempt > 0 && attemptProxyUrl !== proxyUrl) {
      console.log(`[TM-Playwright] Rotated nsocks session for retry ${attempt + 1}: ${attemptProxyUrl?.substring(0, 70)}...`);
    }
    ...
    if (isBrowserAPI) {
      ...
      if (proxyUrl && proxyUrl.startsWith("http")) {
        try {
          ...
          // Strip proxy web-bypass flag (-opt-wb) from username — it blocks Playwright's CONNECT tunnel
