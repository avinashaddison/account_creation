...
import {
  createAccount as _smtpDevCreateAccount,
  deleteAccount as _smtpDevDeleteAccount,
...
// ── Business Mail (smtp.dev / addison.asia) ───────────────────────────────────
...
async function getBizAccountAndInbox(email: string): Promise<{ accountId: string; inboxId: string } | null> {
  try {
    const accounts = await _smtpDevListAccounts();
    const acct = accounts.find(a => a.address.toLowerCase() === email.toLowerCase());
    if (!acct) return null;
    const inbox = acct.mailboxes.find(m => m.path === "INBOX") ?? acct.mailboxes[0];
    if (!inbox) return null;
    return { accountId: acct.id, inboxId: inbox.id };
  } catch {
    return null;
  }
}
...
export async function createBizMailAccount(opts: {
  ...
  if (opts.customUsername) {
    ...
    const nums = all.map((a: any) => a.accountNum).filter((n: any): n is number => n !== null);
    accountNum = nums.length > 0 ? Math.max(...nums) + 1 : 1;
    username = `account${accountNum}`;
  }
  ...
  try {
    const { account } = await _smtpDevCreateAccount(email, password);
    ...
    console.log(`[BizMail] Created smtp.dev account: ${email} (id: ${account.id})`);
  } catch (err: any) {
    const msg = (err.message || "").toLowerCase();
    if (msg.includes("already") || msg.includes("exists") || msg.includes("conflict")) {
      console.log(`[BizMail] ${email} already exists on smtp.dev — reactivating in DB`);
      const existing = await storage.getBizMailByEmail(email);
      if (existing) {
        await storage.reactivateBizMailAccountByEmail(email, password);
      } else {
...
export async function deleteBizMailAccount(email: string): Promise<void> {
  try {
    const accounts = await _smtpDevListAccounts();
    const acct = accounts.find((a: any) => a.address.toLowerCase() === email.toLowerCase());
    if (acct) {
      await _smtpDevDeleteAccount(acct.id);
      console.log(`[BizMail] Deleted smtp.dev account: ${email}`);
    } else {
      console.warn(`[BizMail] Account not found on smtp.dev: ${email}`);
    }
  } catch (err: any) {
    console.warn(`[BizMail] Delete error for ${email}: ${err.message}`);
  }
}

// ── Forwarder stubs (smtp.dev does not have a forwarder API) ─────────────────
...
export async function deleteBizMailForwarder(_forwarderId: string): Promise<void> {
  console.warn("[BizMail] Forwarders not supported with smtp.dev");
}
...
// ── No-op stubs for JMAP / webmail routing (handled automatically by smtp.dev) ─

export async function ensureBizMailJmapRouting(_bizEmail: string): Promise<void> {}

export async function setupAllBizMailJmapRouting(): Promise<void> {
  console.log("[BizMail] smtp.dev handles routing automatically — no setup needed");
}

export async function registerBizMailWebmail(
  _email: string,
  _password: string,
  _displayName: string,
): Promise<boolean> {
...
export async function pollBizMailInbox(
  ...
  const loc = await getBizAccountAndInbox(email);
  if (!loc) {
    console.warn(`[BizMail] smtp.dev account not found: ${email}`);
    return;
  }
  const { accountId, inboxId } = loc;
  console.log(`[BizMail] Polling smtp.dev inbox for ${email}...`);

  while (!shouldStop() && Date.now() < deadline) {
    try {
      const messages = await _smtpDevListMessages(accountId, inboxId, 30);
      for (const m of messages) {
        if (seenIds.has(m.id)) continue;
        seenIds.add(m.id);
        const msgDate = new Date(m.date).getTime();
        if (msgDate < since.getTime() - 30_000) continue;

        const full = await _smtpDevGetMessage(accountId, inboxId, m.id);
        const body = (full?.html || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()
          || full?.text || m.intro || "";
        await onMessage({
          uid: 0,
          from: m.from,
      ...
      console.log(`[BizMail] smtp.dev poll error: ${err.message}`);
    }
    await new Promise(r => setTimeout(r, 20_000));
  }
}

// ── Poll smtp.dev inbox for OpenAI / ChatGPT verification email ────────────────
...
export async function fetchOpenAICodeFromBizMail(
  ...
  const loc = await getBizAccountAndInbox(email);
  if (!loc) {
    log(`[BizMail] smtp.dev account not found: ${email}`);
    return null;
  }
  const { accountId, inboxId } = loc;
  log(`[BizMail] Polling smtp.dev inbox for ${email}...`);
    ...
    try {
      ...
      for (const m of messages) {
        ...
        const isOpenAI =
          ...
          (m.intro || "").toLowerCase().includes("openai");
        if (!isOpenAI) continue;

        log(`[BizMail] Found email from="${m.from}" sub="${m.subject}"`);

        const full = await _smtpDevGetMessage(accountId, inboxId, m.id);
        const rawBody = full?.html || full?.text || m.intro || "";
      ...
      log(`[BizMail] smtp.dev poll error: ${err.message}`);
    }
  ...
  log(`[BizMail] Timed out waiting for OpenAI verification email to ${email}`);
  return null;
}
