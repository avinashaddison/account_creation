      ...
      <div className="flex items-center justify-center py-20">
        <div className="flex flex-col items-center gap-3">
          <div className="relative">
            ...
            <Loader2 className="w-8 h-8 animate-spin text-emerald-400/60 relative" />
          </div>
          <p className="text-[10px] text-emerald-400/30 font-mono uppercase tracking-wider">Loading systems...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5 animate-float-up">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2.5">
            <Cpu className="w-5 h-5 text-emerald-400/50" />
            <h1 className="text-xl font-bold tracking-tight text-white font-mono" data-testid="text-dashboard-title">
              System<span className="text-emerald-400">_</span>Overview
            </h1>
          </div>
          <p className="text-emerald-400/30 mt-1 text-[11px] font-mono pl-7.5">Realtime operational metrics</p>
        </div>
        {data?.role === "superadmin" && (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md" style={{ background: 'rgba(0,255,65,0.05)', border: '1px solid rgba(0,255,65,0.1)' }}>
            <Shield className="w-3 h-3 text-emerald-400" />
            <span className="text-[10px] font-mono text-emerald-400 uppercase tracking-wider">Root Access</span>
          </div>
        )}
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "TOTAL_ACCOUNTS", value: total, color: "emerald", iconColor: "text-emerald-400", valueColor: "text-emerald-300", icon: Database, sub: `$${(data?.billingTotal || 0).toFixed(2)} invested` },
          ...
          >
            <div className="relative">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[9px] font-mono text-emerald-400/35 uppercase tracking-[0.15em]">{stat.label}</span>
                <div className="p-1.5 rounded-md" style={{ background: `rgba(0,255,65,0.04)`, border: '1px solid rgba(0,255,65,0.08)' }}>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <div className="rounded-lg cyber-card p-5" data-testid="card-success-ring">
          <h3 className="text-[9px] font-mono text-emerald-400/35 uppercase tracking-[0.15em] mb-5 flex items-center gap-2">
            <BarChart3 className="w-3 h-3 text-emerald-400/30" /> Success_Metric
          </h3>
          <div className="flex flex-col items-center">
            <div className="relative w-32 h-32">
              <svg className="w-32 h-32 -rotate-90" viewBox="0 0 120 120">
                <circle cx="60" cy="60" r="52" fill="none" stroke="rgba(0,255,65,0.04)" strokeWidth="8" />
                <circle
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-2xl font-bold text-emerald-300 font-mono">{successRate}%</span>
                <span className="text-[8px] text-emerald-400/30 font-mono uppercase tracking-wider mt-0.5">success</span>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2 mt-4 w-full text-center">
              {[
                { val: verified, label: "PASS", color: "text-emerald-400", bg: "rgba(0,255,65,0.04)" },
                ...
                <div key={s.label} className="p-2 rounded-md" style={{ background: s.bg, border: '1px solid rgba(0,255,65,0.05)' }}>
                  <div className={`text-sm font-bold ${s.color} font-mono`}>{s.val}</div>
                  <div className="text-[7px] text-emerald-400/25 font-mono uppercase tracking-wider mt-0.5">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {data?.role !== "superadmin" ? (
          <div className="lg:col-span-2 rounded-lg cyber-card p-5 relative overflow-hidden" data-testid="card-wallet-balance">
            <h3 className="text-[9px] font-mono text-emerald-400/35 uppercase tracking-[0.15em] mb-4 flex items-center gap-2">
              <Wallet className="w-3 h-3 text-emerald-400/30" /> Wallet_Balance
            </h3>
            <div className="mb-4">
              <div className="text-4xl font-bold text-emerald-300 tracking-tight font-mono">${walletBalance.toFixed(2)}</div>
              <p className="text-[11px] text-emerald-400/30 mt-1.5 font-mono">
                {walletBalance > 0
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="p-3 rounded-md" style={{ background: 'rgba(0,255,65,0.03)', border: '1px solid rgba(0,255,65,0.06)' }}>
                <div className="text-[8px] text-emerald-400/30 font-mono uppercase tracking-wider">Unit Cost</div>
                <div className="text-base font-bold text-zinc-200 mt-1 font-mono">${accountPrice.toFixed(2)}</div>
              </div>
              <div className="p-3 rounded-md" style={{ background: 'rgba(0,255,65,0.03)', border: '1px solid rgba(0,255,65,0.06)' }}>
                <div className="text-[8px] text-emerald-400/30 font-mono uppercase tracking-wider">Total Spent</div>
          ...
          <div className="lg:col-span-2 rounded-lg cyber-card p-5 relative overflow-hidden">
            <h3 className="text-[9px] font-mono text-emerald-400/35 uppercase tracking-[0.15em] mb-4 flex items-center gap-2">
              <Terminal className="w-3 h-3 text-emerald-400/30" /> System_Status
            </h3>
            <div className="grid grid-cols-2 gap-2">
              {[
                { icon: DollarSign, label: "Revenue", value: `$${(data?.billingTotal || 0).toFixed(2)}`, color: "emerald" },
                { icon: Users, label: "Accounts", value: `${total}`, color: "amber" },
                ...
                <div key={item.label} className="p-3 rounded-md" style={{ background: `rgba(0,255,65,0.03)`, border: '1px solid rgba(0,255,65,0.06)' }}>
              ...
              {smsPoolBalance !== null && (
                <div className="p-3 rounded-md" style={{ background: 'rgba(0,255,65,0.03)', border: '1px solid rgba(0,255,65,0.06)' }} data-testid="card-smspool-balance">
                  <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <div className="text-[8px] text-emerald-400/20 font-mono mt-0.5">Phone verification</div>
                </div>
              )}
              {capSolverBalance !== null && (
                <div className="p-3 rounded-md" style={{ background: 'rgba(0,255,65,0.03)', border: '1px solid rgba(0,255,65,0.06)' }} data-testid="card-capsolver-balance">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <Zap className="w-3.5 h-3.5 text-amber-400" />
                    <span className="text-[9px] font-mono text-amber-400/60 uppercase tracking-wider">CapSolver</span>
                  </div>
                  <div className="text-xl font-bold text-amber-300 font-mono">${parseFloat(capSolverBalance).toFixed(2)}</div>
                  <div className="text-[8px] text-emerald-400/20 font-mono mt-0.5">CAPTCHA solving</div>
                </div>
              )}
            </div>
          </div>
      ...
      {total > 0 && (
        <div className="rounded-lg cyber-card p-5">
          <h3 className="text-[9px] font-mono text-emerald-400/35 uppercase tracking-[0.15em] mb-3 flex items-center gap-2">
            <Activity className="w-3 h-3 text-emerald-400/30" /> Pipeline_Distribution
          </h3>
          <div className="h-2 rounded-full overflow-hidden flex" style={{ background: 'rgba(0,255,65,0.04)' }}>
            {drawOkCount > 0 && (
              <div
                className="h-full transition-all duration-700"
                style={{ width: `${(drawOkCount / total) * 100}%`, background: 'linear-gradient(90deg, #00ff41, #00cc33)', boxShadow: '0 0 8px rgba(0,255,65,0.3)' }}
            ...
            {verifiedOnlyCount > 0 && (
              <div
                className="h-full transition-all duration-700"
                style={{ width: `${(verifiedOnlyCount / total) * 100}%`, background: 'linear-gradient(90deg, #00ff41, #06b6d4)', boxShadow: '0 0 8px rgba(0,255,65,0.3)' }}
            ...
            {drawRegCount > 0 && (
              <div
                className="h-full transition-all duration-700"
                style={{ width: `${(drawRegCount / total) * 100}%`, background: 'linear-gradient(90deg, #ffb000, #8b5cf6)', boxShadow: '0 0 8px rgba(255,176,0,0.3)' }}
            ...
            {(pending - drawRegCount) > 0 && (
              <div
                className="h-full transition-all duration-700"
                style={{ width: `${((pending - drawRegCount) / total) * 100}%`, background: 'linear-gradient(90deg, #ffcc00, #ff9900)', boxShadow: '0 0 8px rgba(255,170,0,0.3)' }}
            ...
            {failed > 0 && (
              <div
                className="h-full transition-all duration-700"
                style={{ width: `${(failed / total) * 100}%`, background: 'linear-gradient(90deg, #ff0033, #cc0022)', boxShadow: '0 0 8px rgba(255,51,102,0.3)' }}
                title={`Failed: ${failed}`}
              />
            )}
          </div>
          <div className="flex items-center gap-4 mt-2.5 flex-wrap">
            {[
              ...
              { label: "Verified", count: verifiedOnlyCount, color: "#00ff41" },
              { label: "Draw Reg", count: drawRegCount, color: "#ffb000" },
      ...
      {recentAccounts.length > 0 && (
        <div className="rounded-lg cyber-card p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[9px] font-mono text-emerald-400/35 uppercase tracking-[0.15em] flex items-center gap-2">
              <Clock className="w-3 h-3 text-emerald-400/30" /> Recent_Operations
            </h3>
            <a href="/admin/accounts" className="text-[10px] text-emerald-400/50 hover:text-emerald-400 flex items-center gap-1 transition-colors font-mono" data-testid="link-view-all-accounts">
          ...
          <div className="space-y-0.5">
            {recentAccounts.map((acc) => (
              <div
                key={acc.id}
                className="flex items-center justify-between p-2.5 rounded-md hover:bg-emerald-500/[0.02] transition-colors group"
                style={{ borderBottom: '1px solid rgba(0,255,65,0.03)' }}
                data-testid={`row-recent-${acc.id}`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-1.5 h-1.5 rounded-full ${
                    acc.status === "completed" ? "bg-emerald-400 shadow-[0_0_4px_#00ff88]" :
                    acc.status === "verified" ? "bg-emerald-400 shadow-[0_0_4px_#00ff41]" :
                    acc.status === "failed" ? "bg-red-400 shadow-[0_0_4px_#ff3366]" :
                    acc.status === "draw_registering" ? "bg-amber-400 shadow-[0_0_4px_#ffb000] animate-pulse" :
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-emerald-400/30 font-mono">{acc.email}</span>
                      <button
                        onClick={() => { sounds.click(); copyEmail(acc.email); }}
                        className="text-emerald-400/10 hover:text-emerald-400/50 transition-colors opacity-0 group-hover:opacity-100"
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-[9px] font-mono px-2 py-0.5 rounded-sm ${
                    ...
                    acc.status === "verified" ? "bg-emerald-400/8 text-emerald-400 border border-emerald-400/15" :
                    acc.status === "failed" ? "bg-red-400/8 text-red-400 border border-red-400/15" :
                    acc.status === "draw_registering" ? "bg-amber-400/8 text-amber-400 border border-amber-400/15" :
